...
void SOGdensity::sogDensity (const std::vector<std::vector<double>>& QRparameters, std::vector<double>& sogdensity, double h) const
{
	...
	const double gamma = sqrt((2./3.)*(Parameters::rms*Parameters::rms));
	...
	for (unsigned int r = 0 ; r < NSteps + 1; ++r)
	{
		...
		for (unsigned int i = 0; i < 11; ++i)
		{
			...
			const double exp1 = exp(-((radiusx - QRparameters[i][0])/beta)*((radiusx - QRparameters[i][0])/beta));
			const double exp2 = exp(-((radiusx + QRparameters[i][0])/beta)*((radiusx + QRparameters[i][0])/beta));
