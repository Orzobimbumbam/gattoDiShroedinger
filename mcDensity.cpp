...
void NuclearDensityWithMC::benchmarkDensity(const std::vector<std::vector<double>>& mcDensity, double h)//h is optional here
{
    ...
    if (arrayOfKeys.size() == NRows)
    {
        ...
        for (unsigned long i = 0; i < NRows; ++i)
            m_benchmarkDensity.insert(std::make_pair(arrayOfKeys[i], mcDensity[i][1]));
    }
    else
        throw std::out_of_range("NuclearDensityWithMC::benchmarkDensity : mismatching key-value size.");
}
